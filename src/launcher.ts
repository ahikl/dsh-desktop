 ...
 */
export function resolveTargetUrl(options: LaunchOptions): string {
  if (options.url !== undefined) return options.url
  if (options.webServerPort !== undefined) return `http://${LOOPBACK_HOST}:${String(options.webServerPort)}`
  return DASHBOARD_URL
}

/** Electron binary basenames to probe on the PATH, per platform. */
function electronBinaryCandidates(platform: NodeJS.Platform): readonly string[] {
 ...
 */
function findOnPath(name: string, pathEntries: string): string | undefined {
  if (pathEntries === '') return undefined
...
export function resolveElectronBinary(
  ...
  requireElectron: (specifier: string) => unknown = specifier => createRequire(import.meta.url)(specifier),
...
export function buildElectronArgs(
  extraArgs: readonly string[] = [], script: string = ELECTRON_MAIN_SCRIPT,
): string[] {
...
export function spawnElectron(
  binary: string, args: readonly string[], env: NodeJS.ProcessEnv, spawnImpl: typeof spawn = spawn,
): ChildProcess {
